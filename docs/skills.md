...
| a threshold is measured, not inherited | Any number that gates behaviour on a model's confidence belongs to **one rubric asked of one model** and does not travel. `ccas.evals.confidence.sweep` is the instrument: it splits a graded run at each candidate cutoff and reports coverage, accuracy above it, and — because "unsure" is not "no idea" — whether the deferred rows still had the right L1 category. `recommend` returns the *lowest* cutoff clearing an accuracy floor, and returns `None` rather than name one backed by fewer than five rows. Both router harnesses print it in the same shape so two models can be compared by eye. |
| a judge dimension is a rubric | `ccas.evals.judge_rubric` states each `JudgeDimension` as an ordered set of level descriptions, low end first, so a model that cannot write prose can still answer it. The rationale is then authored **in code** from the level chosen, which is stronger than a generated one: it is the rubric's own words and cannot describe a level the model did not pick. `pii_leakage` has no rubric and never will — judging it means sending pre-redaction text to a vendor (Rule 2). |
